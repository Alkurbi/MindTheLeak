...
2. حوّل التسريبات الصغيرة إلى أرقام سنوية ("23 ر.س أسبوعياً = 1196 ر.س سنوياً"). لا تقل أبداً "فقط X ريال يومياً".
3. دقة مرحة بلا وعظ: "رابع طلب هذا الأسبوع، الساعة 2:07 فجراً" وليس "أنت مسرف".
...
- narrativeAr: تقرير من 3 فقرات قصيرة: (1) النمط الأبرز وآليته النفسية، (2) التسريبات الأخرى بالأرقام، (3) الصورة الكبيرة: ماذا يعني استرداد هذا المبلغ سنوياً بشكل ملموس (رحلة عمرة، قسط سيارة، صندوق طوارئ).
- tipsAr: 3 أفعال محددة مرتبة بالأثر، كل واحد بصيغة "افعل X → توفر Y ر.س شهرياً".`;
...
function templateNarrative(
  ...
  const headlineInsightAr =
    stats.paydayWindowSharePct >= 30 && stats.lateNightCountPerMonth >= 3
      ? `${stats.paydayWindowSharePct}٪ من إنفاقك يحدث في أول 5 أيام بعد الراتب، ومعظم قراراتك المتهورة تُتخذ بعد الساعة 10 مساءً (${Math.round(stats.lateNightCountPerMonth)} عملية شهرياً).`
      : top
      ? top.detailAr
  ...
  const narrativeAr = [
    ...
    leaks
      ...
      .map((l) => l.detailAr)
      .join(" "),
    `المجموع: ${fmt(monthlyLeakSar * 12)} ر.س سنوياً قابلة للاسترداد.${goalSentence} الأبحاث تظهر أن مجرد رؤية هذه الأرقام يخفض الإنفاق الاستهلاكي بنسبة 11٪ خلال سنة.`,
  ].join("\n\n");
  ...
  const tipsAr = leaks
    ...
    .map((l) => {
      ...
      switch (l.kind) {
        case "late_night":
          return `أخّر أي شراء بعد 10 مساءً إلى صباح اليوم التالي (قاعدة الـ 12 ساعة) → توفر حتى ${save} ر.س شهرياً`;
        case "delivery_streak":
          return `حدد سقف التوصيل بطلبين أسبوعياً → توفر ${save} ر.س شهرياً`;
        case "forgotten_subscription":
          return `راجع اشتراكاتك اليوم وألغِ ما لم تستخدمه آخر 30 يوماً → توفر حتى ${save} ر.س شهرياً`;
        case "payday_spike":
